...
It provide also a \verb|toList| function, but WARNING, this is not what you think! It literally transform a graph into a list. If you have a self-loop or any time two vertices, then you will have two times the same vertex.

You can use the \verb|Foldable| instance by thinking a graph like a list. For example,
...
Alga is providing a faster \verb|toList| (called \verb|vertexList|) based on \verb|vertexSet| and thus requiring an \verb|Ord| instance on the vertices. Moreover, \verb|vertexList| give vertices in \emph{ascending order} and without duplicates.
\\
Keep this in mind:
\begin{lstlisting}[language=Haskell, frame=single]
toList /= vertexList
